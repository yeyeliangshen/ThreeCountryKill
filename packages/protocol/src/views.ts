...
export interface PlayerView {
  ...
  inFormation?: boolean;
  /**
   * **本回合被【调虎离山】移出座次**（公开信息：牌面文本就写着「不计入距离和座次的计算」，
   * 结算日志也点名了谁被移出）。界面据此画一枚很轻的标记——没有它，牌桌上**看不出**
   * 谁不在座次里（距离/队列/围攻都跟着变了，玩家只能靠日志回忆）。
   */
  removedFromSeating?: boolean;
  /**
   * **这个角色此刻是否拥有【飞影】**（＝其他角色计算与他的距离 +1）。
   *
   * 来源可能**不是他自己的武将牌**——【鹤翼】（曹洪，阵法技）两态互斥：
   * - 常规（曹洪没进队列）：**曹洪自己**有飞影；
   * - 队列：**改为**同队列的其他角色有飞影，曹洪自己反而没有了。
   *
   * `feiyingFrom` 给出**赋予者**的座位（用于界面写清来源：「来源：【鹤翼·曹洪】」——
   * 用户 2026-09-26 口径：玩家需要知道这不是他自己的武将技）。曹洪常规态时等于他自己。
   */
  feiying?: boolean;
  feiyingFrom?: string;
  /**
   * **【鹤翼】此刻的形态**（只对持有【鹤翼】的角色下发）：
   * - `'normal'`：视为自己拥有【飞影】；
   * - `'formation'`：与同队列的其他角色共享（飞影给他们，自己不再享受）。
   *
   * 判据在引擎（`heroes.heyiInFormation`），界面只显示；两态**互斥**，别写成「全员都有」。
   */
  heyiMode?: 'normal' | 'formation';
  /**
   * **当前公开的性别**（界面上的 ♂ / ♀ / ?）：
...
export interface PromptView {
  ...
  /**
   * 与这条询问有关的**其他角色**（纯展示）：界面把它**轻微高亮**，但不改变可点性。
   *
   * 例：徐盛·【疑城】问徐盛时 = 被保护的那位（用户 2026-09-25 口径：一眼看出保护的是谁）。
   */
  relatedSeats?: string[];
  pickCards?: Card[];
  /**
