 ...
 *
 * **全部装备的特效都已实现**：诸葛连弩、青釭剑、七星宝刀、古锭刀、寒冰剑、麒麟弓、
 * 吴六剑、三尖两刃刀、八卦阵、仁王盾、藤甲、白银狮子、贯石斧、青龙偃月刀、雌雄双股剑、
 * 丈八蛇矛、明光铠、护心镜、玉玺、木牛流马、方天画戟（**国战版**），以及马术类的距离修正。
 *
 * 只剩**军争版方天画戟**（「最后一张手牌可额外指定两个目标」）没做——国战版做了另一套。
 */
const EQUIP_NOT_IMPLEMENTED: ReadonlySet<string> = new Set([
  // 军争的方天画戟（「最后一张手牌可额外指定两个目标」）没做，
  // 国战版做了——它走的是另一套文本与另一套目标规则，见下。
  'fangtian',
]);

/** 国战模式下仍未实现特效的装备：**没有**了（方天画戟与丈八蛇矛都已按国战文本实现） */
