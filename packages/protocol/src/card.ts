...
/**
 * 「伤害类卡牌」：**能直接造成伤害的卡牌**（用户 2026-09-18 给的定义）。
 *
 * 例：【杀】（含火杀/雷杀，属性只是伤害的属性）、【决斗】、【南蛮入侵】、【万箭齐发】、【火攻】、
 * 【水淹七军】、【火烧连营】。用于君袁绍·授锋（「当你于其出牌阶段使用首张伤害牌结算结束后」，
 * 原文见 docs/guozhan-roster.md §5.60）。
 *
 * ⚠️ **不算**的：
 *   - 【酒】（它本身不造成伤害，只给下一张【杀】加伤害）；
 *   - 【闪电】（判定阶段才结算伤害，不是「使用时直接造成伤害」）；
 *   - 【借刀杀人】（让别人去用【杀】，自己不打伤害）。
 *
 * ⚠️ 这里**翻过一次口径**，如实记：早先按「使用之后**可能**造成伤害」的宽口径，把【闪电】也算进来了
 * （当时的注释写着「用户 2026-09 裁定」，理由是有技能文本写「除【闪电】外的伤害类卡牌」）。
 * 2026-09-18 用户明确按「**直接**造成伤害」定义、并点名【酒】【闪电】**不算** → 已把【闪电】移出。
 * ⇒ 将来若真的遇到牌面写「除【闪电】外的伤害类卡牌」的技能，回来对一次（那种写法本身暗示闪电算）。
 */
 ...
 * - 另一条特殊规则（**移动版口径**，用户 2026-09 明确）：这类牌**要进弃牌堆时改为销毁**
 *   （＝移出游戏，不再回到任何牌区）。用户特别指出：网上「势力锦囊改为进入**府库**」是
 *   **另一套规则集（线下面杀）**的说法，移动版不建那个区；本仓库按移动版实现。
 *
...
export const EQUIP_NAME: Record<string, string> = {
  ...
  // 【定澜夜明珠】的文本（用户核对）：「锁定技，你每回合首次弃置牌后，摸一张牌。当此牌离开你的
  // 装备区时，销毁之。」——**它已从正常牌堆移除**（只能通过【君威】从游戏外取得），
  // 君主将（含君孙权）也都已实现；这段注释以前写着「君孙权整只还没做」，早已不成立。
  feilong: '飞龙夺凤',
  liulong: '六龙骖驾',
  mengjun: '盟军大纛',
  dinglan: '定澜夜明珠',
...
export const CARD_DESC: Record<CardType, string> = {
  ...
  xietianzi:
    '出牌阶段，若你是大势力角色，对你使用。结束你的出牌阶段，然后若你于弃牌阶段弃置一张手牌，则于此回合结束后，你进行一个额外的回合。',
  lutong:
  ...
  // 延时锦囊
  // ⚠️【乐不思蜀】**没有**距离限制（用户 2026-09-22 口径）——原文案写的「（距离≤1）」是错的，
  // 与引擎的距离判据一起修（见 docs/guozhan-roster.md §5.208）；距离限制只属于【兵粮寸断】。
  lebu: '置于一名其他角色的判定区（无距离限制）。其判定阶段判定：若不为红桃，跳过其出牌阶段。',
  shandian:
...
/**
 * **「这张牌的目标能不能是使用者本人」——每张牌自己的目标规则**（唯一事实来源）。
 *
 * 口径（用户 2026-09-24 给的缺陷与原则，规则权威，逐字记入 `docs/guozhan-roster.md` §5.209）：
 *
 *   目标选择系统**不应**存在「所有技能或卡牌默认不能选择自己」的全局规则，而应严格根据每张牌、
 *   每个技能的具体目标描述判断。核心原则：**是否能够选择自己必须由牌或技能文本决定，而不能由
 *   通用目标选择 UI 决定**。牌面写「一名**其他**角色」的 ⇒ 必须排除自己；写「一名角色」（没有
 *   「其他」）的 ⇒ 不能因为通用逻辑自动禁选自己。
 *
 * 所以本表是一张**声明表**，不是通用规则：
 *   - **在表里** ⇒ 这张牌**自己**声明了「排除自己」（依据＝牌面/用户口径里的「其他角色」写法）；
 *   - **不在表里** ⇒ 这张牌**没有**声明排除自己 ⇒ 使用者自己**可以被指定**（前提照牌面文本走：
 *     例如【火攻】还要求目标「有手牌」，那是另一条判据 `trickNeedsCardsInHand`）。
 *
 * 消费者：引擎的出牌校验（`playSha` / `playTrick` / `playDelayedTrick`）、引擎的候选与提示下发
 * （`legal.ts` 的 `selfTargetUses`）、界面（读服务端下发的自身合法目标）。三处读的都是这一份。
 *
 * ⚠️ **逐条依据**（改任何一条前先核文本；核不到就标 `待核对`，别猜）：
 * | 牌 | 牌面/口径 | 判定 |
 * |---|---|---|
 * | 【杀】 | **用户 2026-09-24 明确点名**：写「其他角色」的牌必须排除自己（举例点到了【杀】） | 排除（在表里） |
 * | 【决斗】【过河拆桥】【顺手牵羊】【乐不思蜀】【兵粮寸断】【知己知彼】【调虎离山】【水淹七军】 | 本地牌面文本写「一名**其他**角色」 | 排除（在表里） |
 * | 【火攻】 | 牌面「一名**有手牌**的角色」（没有「其他」）——用户 2026-09-24 点名的第一条：只要自己有手牌，可以对自己使用 | **不排除**（不在表里） |
 * | 【铁索连环】 | 牌面「一至两名角色」（没有「其他」） | **不排除** |
 * | 【号令天下】 | 口径见 §5.136.3「目标可以包括自己」 | **不排除** |
 * | 【克复中原】 | 牌面「对至少一名角色使用」（没有「其他」） | **不排除** |
 * | 【挟天子以令诸侯】【固国安邦】 | 只对自己使用（目标由规则定死，不经玩家指定） | 不适用（不在表里） |
 * | 【远交近攻】 | 「与你**势力不同**」——自己与自己势力相同 ⇒ 文本自身就排除了自己 | 不适用（不在表里） |
 * | 【联军盛宴】 | 「你选择一个**其他势力**」——同一条理：条件即排除自己 | 不适用（不在表里） |
 * | 【南蛮入侵】【万箭齐发】【桃园结义】【五谷丰登】【火烧连营】【勠力同心】【敕令】【文和乱武】【以逸待劳】 | 目标由规则算出来，玩家不指定 | 不适用（不在表里） |
 * | 【借刀杀人】 | ⚠️ **待核对**：本地文案缺「其他」二字，官方通行口径含「其他角色」；本轮**保持**既有行为（候选只给其他人、引擎未加自身校验），见 §5.209 待核对项 | 待核对 |
 *
 * ⚠️ 【杀】的**本地文案**（`CARD_DESC.sha`）写的是「一名角色」、缺「其他」二字——与上表的判定
 *    不一致。本轮**不改文案**（文案属规则文本，未核到官方逐字原文前不动它），已在 §5.209 记为
 *    待核对；判定按用户口径走「排除自己」。
 */
export const CARD_TARGET_EXCLUDES_SELF: ReadonlySet<CardType> = new Set<CardType>([
  'sha',
  'juedou',
  'guohe',
  'shunshou',
...
/**
 * 这张牌**自己**声明了「排除自己」吗（纯函数，不看局面）。
 *
 * 用 `CardType`（而不是 `Card`）是因为**转化后的牌型**才算数：卧龙·火计把红牌当【火攻】用时，
 * 判的必须是【火攻】的目标规则，而不是那张红牌本来的牌型——出牌校验那几处传的就是
 * `as ?? card.type` 算出来的**生效牌型**。
 */
export function cardTargetExcludesSelf(type: CardType | null | undefined): boolean {
  return type != null && CARD_TARGET_EXCLUDES_SELF.has(type);
}

/** 反过来读：这张牌的文本**允许**把使用者自己选为目标（未声明排除自己）。 */
export function cardTargetAllowsSelf(type: CardType | null | undefined): boolean {
  return !cardTargetExcludesSelf(type);
}
