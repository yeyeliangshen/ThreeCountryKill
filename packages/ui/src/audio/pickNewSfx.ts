...
/**
 * 给定上次处理到的日志 id 与本次快照里的日志窗口，返回要播的音效与新游标。
 *
...
export function pickNewSfx(prevId: number, log: LogEntry[] | undefined): SfxPick {
  if (!log || log.length === 0) return { name: null, lastId: -1 };
  const newest = log[log.length - 1]!.id;
  // 首次拿到日志（进局 / 重连）：只对齐游标，不补放声音，
  // 否则一进局就会把开局那几条一起响出来
  if (prevId < 0) return { name: null, lastId: newest };

  // 一次快照可能带多个事件（南蛮入侵结算一整轮等），只取最后一个有音效的，
  // 否则会糊成一片
