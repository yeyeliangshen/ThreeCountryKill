...
describe('随机对局不变式：曾经出过问题的种子（回归）', () => {
   ...
   * - `seed=1999`：弃牌阶段的占位被【屯田】的问话覆盖之后流程收尾没回来，pending 停在 null
   *   （兜底当时只认出牌阶段）→ 整局卡死。现在兜底也覆盖弃牌阶段（`beginDiscard` 按当前
   *   手牌重算，已经弃过不会再来一次）。
...
// 这条网盯的是「结算被积压」：某条被打断的流程忘了接着跑，牌一直攥在闭包里没人放回去。
//
// 已经修掉的现场（都在 §5.43–§5.47 有记录）：
//   ① 续接队列的**排序**（内层续接要插队首）：判定牌被搁置 27-44 步；
//   ② 「判定链已经结束后写回的替换牌」凭空消失（`JudgeBox.chain.dead` 兜底）；
//   ③ 用牌的两处重复弃置（`takeAndDiscard` / `takeForField`）；
//   ④ 判定阶段在飞的牌在判定者阵亡后没人管（`GameState.judgmentInFlight` 台账）；
//   ⑤ 续接队列**只在 `pending === null` 时排**：出牌/弃牌阶段的占位空位被当成了「有人被问话」，
//      队列整步整步地被跳过 —— 这才是「结算拖到十几步之后」的根因。
//      现在 `isIdlePending()` 把**弃牌阶段的占位**也算空场（出牌阶段不算：那正是回合交替
//      正在发生的窗口，见 engine.ts 里那段注释）。
//
// 实测（1500 局，2026-09）：修 ⑤ 之前最长缺席 13 个稳定步、还有两局跑满 6000 步没结束；
// 修完 ⑤ 之后**最长缺席 0 个稳定步、1500 局全部分出胜负**。
