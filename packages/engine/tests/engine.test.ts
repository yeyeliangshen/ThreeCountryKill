...
describe('国战 · SP司马昭（昭心 / 夙智）', () => {
  ...
  // ⚠️ 仍 skip：本轮已经把「其他角色因弃置进弃牌堆」**派给全场**的新时机接上了
  //    （`anyCardDiscarded`），并补了「过河拆桥拆装备」那条不进弃置收口的旧洞，
  //    但这张用例仍然等不到询问——时间不够继续追，留 root cause 给下一轮：
  //    下一步要查的是 handler 里的 `suzhiAvailable`（额度/回合限制）与询问挂起时序。
