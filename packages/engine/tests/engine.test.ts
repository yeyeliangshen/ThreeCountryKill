...
describe('国战 · SP司马昭（昭心 / 夙智）', () => {
  ...
  // ⚠️ 仍 skip：围栏（resumePlay 带 checkpoint）已经能保住**锦囊收尾**里产生的询问（成略那条通了），
  //    但把这条也打开会让冒烟 6 条挂掉——弃置收口里的询问出现在**非锦囊**的许多流程里，
  //    而现在的「排队」用的是 pushResume（只在槽为 null/弃牌阶段时被排空）；流程回到「出牌阶段」
  //    占位时那条续接永远醒不过来 → 卡死。所以必须先做 §5.124 待办 3（drainResume 的唤醒语义）。
...
describe('国战 · 许攸（成略 / 恃才）', () => {
  ...
  // ⚠️ 仍 skip：围栏判断本身是对的（这条用例一度通过），但「被挡住之后怎么醒」还没做——
  //    启用后冒烟 6 条挂（见 docs §5.124.1）。做完唤醒语义再来打开它。
