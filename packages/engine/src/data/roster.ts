...
const BUCHEN: RosterEntry[] = [
  ...
  },
  {
    ...
    note: '雉盗 + 寄篱（不臣篇·上，群，国战牌面 2 阴阳鱼 → 4，称号·豺牙落涧；文本已核）。取 **2021 移动版**口径：寄篱是副将技（deputySlotSkills + deputySlotHalfYang；2022 版去掉了副将技标签与 -1 阴阳鱼，2021 线下实体卡把「再使用一次」写成「此牌结算两次」，同义）。雉盗：距离用现成的 flags.distanceToOneThisTurn，「只能指定他与你」用新标记 flags.cardTargetOnlySeat 并在 engine.onPlayCard 统一拦（zhidaoTargetsBlocked，AOE 那类不指定目标却会打到别人的牌也一并拦）；「第一次对其造成伤害后获得其区域里的一张牌」挂 afterDamageDealt + flags.zhidaoHitDone，拿牌走 takeOneOfTargetCards。寄篱三句都实现了：①「此牌结算两次」——成为红色即时锦囊唯一目标时由 othersBecomeTarget 钩子置 trickCtx.jiliSecond、成为红色【杀】唯一目标（attack.totalTargets===1）时由 becomeTarget 钩子置 attack.jiliSecond；执行侧在单目标锦囊的结算收口 endTrickResolution()（原来 54 处 resumePlay 全换成它）、【杀】的 afterAttackSettledTail、【桃】的自用与濒死求桃两条路上各再结算一次；防递归＝ctx.jiliDone + 按牌 id 的账本 state.jiliReranCards（随回合清空）。②「本阶段第 2 次受到伤害防止并移除」——damageDealt 里读 flags.damageCountKey/count（键＝`回合座位:阶段名`）。⚠️ 已知简化/读法：第二遍结算不沿用第一遍选的明牌（过拆/顺手那张可能已被拿走，让使用者重选）；被无懈抵消的锦囊照样会「再使用一次」（按「成为目标就触发」的字面读法）；【酒】靠布尔标记生效，重跑不叠加伤害。',
  },
...
export const GUOZHAN_ROSTER: RosterEntry[] = [
   ...
   *
   * 已实现：君主特性（见下）+ 君刘备的【君威】与专属装备【飞龙夺凤】（WIKI 原文已核）。
   * 未实现：另外三位君主的【君威】（它们的专属装备效果文本 WIKI 无页面、搜索配额也用尽了，
   * **待核对**）；【六龙骖驾】【定澜夜明珠】【盟军大纛】同理。
   *
   ...
   */
  {
    ...
    pack: 'jun',
    status: 'partial',
    note: '君主特性 + 【君威】+ 专属装备【六龙骖驾】（♥K 宝物：你计算与其他角色的距离 -3；离开装备区即销毁）+ 【雄驰】（每回合第一次造成伤害后，可令受伤的角色对一名与你势力相同的角色造成 1 点虚拟伤害）+ 【征戎】（受到伤害后，可将一名角色的至多 X 张手牌替换为等量张【杀】，X＝你已损失的体力值且至少 1；换来的【杀】是从**牌堆**里找出的实体牌）都已实现——均按用户核对后提供的口径。未实现：【总御】（缺完整文本）。⚠️ 版本差异：用户先前查到的【建安】（五子良将纛）/【挥鞭】/【总御】属于**另一版本**（2019 典藏版 / 君临天下·权），按用户口径不在这个君曹操身上，已从武将上摘掉（实现留在 git 历史里；引擎的「君主旗」机制 lordBanner 保留未删）。',
  },
  {
    id: 'junliubei',
    name: '君刘备',
    ...
    pack: 'jun',
    status: 'partial',
    note: '君主特性 + 【君威】 + 专属装备【飞龙夺凤】（「每回合首次用【杀】造成伤害后获得其一枚阴阳鱼标记或一张手牌」+「离开装备区即销毁」）+ 【章武】（一名角色的结束阶段，可视为使用 1 枚与你势力相同的角色本回合使用过的国战标记——本实现把三枚标记的效果抽成可复用函数，视为使用时不消耗标记；账本 GameState.markerUsesThisTurn 每回合清空）+ 【励众】（锁定技：每轮结束时给本轮造成伤害最多的同势力角色各 1 枚「先驱」；引擎在座次绕回首位时派发 roundEnd + 按轮清空的伤害台账）都已实现。⚠️ 待核对：【阴阳鱼】在弃牌阶段的那条用法（弃置 → 本回合手牌上限 +2）本仓库尚未实现，所以章武的候选里也只有出牌阶段那一版。详见 docs/guozhan-roster.md §5.62。',
  },
  {
    id: 'junsunquan',
    name: '君孙权',
    ...
    pack: 'jun',
    status: 'partial',
    note: '君主特性 + 【君威】+ 专属装备【定澜夜明珠】（锁定技：你每回合首次弃置牌后摸一张；离开装备区即销毁）+ 【督授】（与你势力相同的角色的出牌阶段限一次，可弃置至多两张牌令你摸等量牌）+ 【据江】（锁定技：吴不为大势力时，与你势力相同的角色指定你为目标的非伤害牌额外结算一次，装备牌/延时锦囊/势力锦囊除外）都已实现——均按用户核对后提供的口径。⚠️ 待核对：①【定澜夜明珠】的类型/花色/点数（本实现按「宝物 ♥7」占位）；②【据江】的「势力锦囊牌」完整定义（本实现取挟天子以令诸侯/联军盛宴/勠力同心）。另：技能里直接 toDiscard 的代价弃置（如【君威】）不算「弃置」，不会触发定澜夜明珠与礼让。详见 docs/guozhan-roster.md §5.61。',
  },
  {
    id: 'junyuanshao',
    name: '君袁绍',
    ...
    pack: 'jun',
    status: 'partial',
    note: '君主特性 + 【君威】+ 专属装备【盟军大纛】（受到伤害时弃两张牌防止此伤害，弃的其中一张可以是宝物自己；离开装备区即销毁）+ 【会盟】（锁定技：场上一个势力的角色数 0 ↔ 非0 时摸一张）+ 【授锋】（一名角色于其出牌阶段使用首张伤害牌结算结束后，交给其一张牌（目标是自己则跳过），然后从弃牌堆获得此伤害牌）都已实现——均按用户核对后提供的口径，并已用移动版 WIKI 复核技能文本。⚠️ 待核对：①【盟军大纛】的类型/花色/点数（WIKI 无此牌页，本实现按「宝物 ♣2」占位）；②【会盟】的「角色数」按明置算还是按武将牌本身的势力算（本实现取明置口径）；③【授锋】的「伤害牌」完整定义，以及「该牌已被别人拿走 / 本来就是虚拟牌」时的结算。详见 docs/guozhan-roster.md §5.60。⚠️ 群势力的君主在部分资料里写作「君张角」，本仓库以 biligame 的君威条目为准取「君袁绍」。',
  },
  ...STANDARD_WEI,
  ...STANDARD_SHU,
  ...STANDARD_WU,
