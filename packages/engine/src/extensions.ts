 ...
 *
 * ✅ 后来补齐的：**野心家武将**（SP司马昭/公孙渊/孙綝/界钟会）与**23 名不臣篇武将的技能文本**
 *   都已实现（见 `heroes.ts`）。
 * ✅ 势力锦囊**四张已全部实装**（固国安邦/号令天下/克复中原/文和乱武，2026-09-18）。
 * ✅ **「暴露野心 → 建立新势力」已实装**（2026-09-18，§5.142）——它是不臣篇**野心家武将**的机制，
 *   与「某势力人数超过总玩家数一半 → 该角色成为普通野心家」是**两套独立规则**
 *   （用户专门纠正过：不是同一机制的前后半段）。
 *   ⚠️ 只剩两处官方没写细、本仓库**按座次**取的顺序（多个野心家同时暴露的先后、多个建国邀请的应答次序）
 *   等实机校正，见 docs §5.141 第 7 点。
 *   注：网上常提到的「府库」属于**线下面杀**那套规则集，移动版没有这个区，不作为待办。
 */
