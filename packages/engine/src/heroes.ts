 ...
 *
 * ⚠️ 暴凌（以及它给的【崩坏】）**尚未实现**。原先的理由（「移除副将的武将牌」这套制度）
 *    已经不存在了——`api.removeHeroCard`（移除武将牌）、`api.changeMaxHp`（改体力上限）、
 *    以及伪武将【崩坏】（「只作为被授予的技能存在」）都齐了；真正还缺的是
 *    「**永久丢掉自己武将牌上的一个技能**（暴凌本身是主将技）」这套增删——
 *    现成的「借来的技能」只是临时形态。技能描述里如实标注为 partial。
 */
/**
 ...
 *   然后若其与你势力相同，其可以**变更副将**。
 *
 * **实现状态**：①「防止此伤害、获得其装备区/判定区一张牌」**已实现**（`damageCaused` 钩子）；
 *    ②「若其与你势力相同，其可以**变更副将**」**未实现**——原本缺的地基（从**未加入游戏的**
 *    武将牌堆连续亮将直到与主将势力相同）现在已经有了（`api.changeDeputyHero`，黄祖·袭射
 *    换暗副将走的就是它），只差把这一句接到制蛮的收尾上。散谣已完成，故 roster 里是 partial。
 */
/**
